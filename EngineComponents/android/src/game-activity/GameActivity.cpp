  ...
  switch (work.cmd) {
    ...
    case CMD_SET_WINDOW_FLAGS: {
      code->env->CallVoidMethod(
          code->javaGameActivity, gGameActivityClassInfo.setWindowFlags,
          static_cast<jint>(work.arg1), static_cast<jint>(work.arg2));
    ...
    case CMD_SET_IME_EDITOR_INFO: {
      code->env->CallVoidMethod(
          code->javaGameActivity, gGameActivityClassInfo.setImeEditorInfoFields,
          static_cast<jint>(work.arg1), static_cast<jint>(work.arg2),
          ...
          [](const char *bytes, int len, void *context) {
            ...
            if (len > 0) {
              ...
              if (locals->array != NULL) {
                locals->env->SetByteArrayRegion(locals->array, 0, len,
                                                (const jbyte *)bytes);
              }
            }
          },
...
static void readConfigurationValues(NativeCode *code, jobject javaConfig) {
  ...
  gConfiguration.keyboard =
      code->env->GetIntField(javaConfig, gConfigurationClassInfo.keyboard);
  ...
  gConfiguration.screenLayout =
      code->env->GetIntField(javaConfig, gConfigurationClassInfo.screenLayout);
  gConfiguration.screenWidthDp =
      code->env->GetIntField(javaConfig, gConfigurationClassInfo.screenWidthDp);
...
extern "C" void GameActivity_setImeEditorInfo(
    GameActivity *activity, GameTextInputType inputType,
    GameTextInputActionType actionId, GameTextInputImeOptions imeOptions) {
  NativeCode *code = static_cast<NativeCode *>(activity);
  write_work(code->mainWorkWrite, CMD_SET_IME_EDITOR_INFO, inputType, actionId,
             imeOptions);
}
...
static bool onTouchEvent_native(JNIEnv *env, jobject javaGameActivity,
  ...
  GameActivityMotionEvent_fromJava(env, motionEvent, &c_event, pointerCount,
                                   historySize);
  ...
  for (int type = 0; type < GAMECOMMON_INSETS_TYPE_COUNT; ++type) {
    ...
    if (type == GAMECOMMON_INSETS_TYPE_WATERFALL) {
      jinsets = env->CallObjectMethod(
          code->javaGameActivity, gGameActivityClassInfo.getWaterfallInsets);
    } else {
      jint jtype = env->CallStaticIntMethod(
          gWindowInsetsCompatTypeClassInfo.clazz,
          gWindowInsetsCompatTypeClassInfo.methods[type]);
      jinsets =
          env->CallObjectMethod(code->javaGameActivity,
                                gGameActivityClassInfo.getWindowInsets, jtype);
    }
...
static void onEditorActionNative_native(JNIEnv *env, jobject activity,
...
static const JNINativeMethod g_methods[] = {
    ...
    {"onEditorActionNative", "(JI)V", (void *)onEditorActionNative_native},
};
...
extern "C" int GameActivity_register(JNIEnv *env) {
  ...
  GET_METHOD_ID(gGameActivityClassInfo.finish, activity_class, "finish", "()V");
  ...
  GET_FIELD_ID(gConfigurationClassInfo.hardKeyboardHidden, configuration_class,
  ...
  for (int i = 0; i < GAMECOMMON_INSETS_TYPE_WATERFALL; ++i) {
    GET_STATIC_METHOD_ID(gWindowInsetsCompatTypeClassInfo.methods[i],
                         windowInsetsCompatType_class, methodNames[i], "()I");
  }
...
Java_com_google_androidgamesdk_GameActivity_initializeNativeCode(
    ...
    jstring obbDir, jstring externalDataDir, jobject jAssetMgr,
    jbyteArray savedState, jobject javaConfig) {
    __android_log_write(ANDROID_LOG_INFO, "FlexKit", "Java_com_google_androidgamesdk_GameActivity_initializeNativeCode: Hello!");
...
extern "C" JNIEXPORT jlong JNICALL
Java_com_example_myapplication_MainActivity_initializeNativeCode1(
